...
public class Planeetta {
    ...
    public double etaisyysX(Planeetta toinen) {
        double etaisyysX = Math.abs(this.xSijainti - toinen.getSijaintiX());
        return etaisyysX;
    }

    public double etaisyysY(Planeetta toinen) {
        double etaisyysY = Math.abs(this.ySijainti - toinen.getSijaintiY());
        return etaisyysY;
    }

    public double vetovoima(Planeetta toinen) { //muuta tätä abs
        double vetovoima = Math.abs(6.67384E-11 * (this.massa * toinen.getMassa()) / (this.etaisyys(toinen) * toinen.etaisyys(this)));
        return vetovoima;
    }

    public double vetovoimaX(Planeetta toinen) {
        return Math.abs(vetovoima(toinen) * this.etaisyysX(toinen) / this.etaisyys(toinen));
    }

    public double vetovoimaY(Planeetta toinen) {
        return Math.abs(vetovoima(toinen) * this.etaisyysY(toinen) / this.etaisyys(toinen));
    }
