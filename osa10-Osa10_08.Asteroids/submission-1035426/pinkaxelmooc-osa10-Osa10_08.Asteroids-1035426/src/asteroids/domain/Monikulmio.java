...
sekä lukea kirja "Essential Mathematics for Games and Interactive Applications". <- luen koko ajan


Tässä tehtävässä näyttäisi olevan luokan Monikulmio perintää alukselle ja asteroidille. 
Keskitytään siis niihin. 
päivitys 1.0: Nyt Monikulmioluokkaan on lisätty metodi liiku, joka periytyisi esimerkiksi ufolle,
joka myös liikkuisi. Aluksen ja asteroidin tapauksessa nämä pitää kuitenkin erikseen mainita,
koska ne käyttävät omia muuttujiaan suorituksen aikana.
päivitys 2.0: Siirrettiin getSijainti-metodi superclass Monikulmioon.
 */
public class Monikulmio {
    ...
    public void liiku(long nanosekuntiaViimePaivityksesta) {
        this.sijainti.setX(this.sijainti.getX() /*+ kuinkaPaljonliikutaan.getX()*/);
        this.sijainti.setY(this.sijainti.getY() /*+ kuinkaPaljonliikutaan.getX()*/);
    }
